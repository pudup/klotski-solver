        ...
        while moves:
            ...
            for move in next_move[2]:
                ...
                if klotski_board.board[4][2] == 'd':
                    solutions += 1
                    curr_board_and_moves_taken[1].append([next_move[0], next_move[1], move])
                    solutions_list.append([curr_board_and_moves_taken[1], len(curr_board_and_moves_taken[1])])
                    if len(curr_board_and_moves_taken[1]) < min:
                        min = len(curr_board_and_moves_taken[1])
                        shortest_solution = curr_board_and_moves_taken[1]
                    if len(curr_board_and_moves_taken[1]) > max:
                        max = len(curr_board_and_moves_taken[1])
                        longest_solution = curr_board_and_moves_taken[1]
